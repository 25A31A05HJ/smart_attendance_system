...
def get_registered_student_ids():
    """
    Returns the set of student_id values that actually exist in
    the database right now. Used to skip any leftover/orphaned
    image in faces/ that doesn't belong to a real registration
    (this is what previously caused false matches and
...
def train_model():
    ...
    # Only train on faces that belong to a student who is
    # actually registered right now. Any other file in this
    # folder (a leftover demo image, or a photo left behind
    # after a student record was removed) is skipped so it
    # can never be mistaken for a real match.
    valid_student_ids = get_registered_student_ids()
