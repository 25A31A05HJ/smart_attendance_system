    ...
    if request.method == "POST":
        ...
        try:
            ...
            connection.execute("""
                ...
                (student_id, name, department, student_type, face_file)
                VALUES (?, ?, ?, ?, ?)
            """, (
                student_id,
                name,
...
def attendance():
    ...
    # Per-student total days present (distinct dates, so a student
    # can't be double-counted even if a date has multiple entries),
    # plus the actual list of dates they were present on.
    summary = connection.execute("""
        SELECT
            ...
            (
                SELECT GROUP_CONCAT(sorted_dates.date, ', ')
                FROM (
